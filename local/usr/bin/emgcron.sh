#!/bin/ksh
#$Header: /u999/cvsadmin/cms_rep/repository/local/usr/bin/emgcron.sh,v 1.1 2011/01/13 02:38:14 hongh Exp $
# -----------------------------------------------------------------------------------------------
# Script  : cmscron.sh
# Author  : Haipeng Hong
# Changes :
...
_PRG=${0}
_PRGID=`basename ${0}`
_PRG_HEADER="$Id: emgcron.sh,v 1.1 2011/01/13 02:38:14 hongh Exp $"
...
. /oracle/local/resource/profile_emg_host.env 1>/dev/null
. /oracle/local/resource/${_ACC_SID}/profile_${_ACC_SID}.env

_logfile="/oracle/local/log/${_RT_COMM_ARG1}_`date +%Y%m%d%H%M%S`_${$}.log"
